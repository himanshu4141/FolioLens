...
- **Version bump:** bump `SCHEMA_VERSION` and add migration to the schema-init code path. Today the upgrade is `ALTER TABLE … ADD COLUMN`; old rows have NULL for new columns. Audit #15 noted that a mid-migration crash can leave orphan rows; preserve the serialized write/cleanup boundary when changing this area.
