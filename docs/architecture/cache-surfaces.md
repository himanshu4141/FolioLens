...
- **Current buster (C1):** `__BUSTER__` is **v9**. v8 added `scheme_active` to the persisted fund payload. v9 intentionally purges persisted historical NAV-derived results because pre-C1 Portfolio timeline and Fund Detail entries may have been computed from recent-only SQLite slices. This semantic-correctness purge supersedes the older version notes above; no payload shape changed.
