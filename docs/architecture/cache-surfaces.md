...
- **CAS C1 holding-activation recency:** the atomic CAS database writer now resolves `user_fund.is_active` through a service-role-only database policy that checks balance recency before positive, zero, or missing closing units. The client `user-funds` payload and every query key remain byte-for-byte unchanged; only an incorrect server boolean is prevented. Direct upload already refreshes user funds and Portfolio, while server-imported changes use the existing user-fund/transaction freshness and invalidation fan-out. No `__BUSTER__`, SQLite `SCHEMA_VERSION`, Zustand version, or AsyncStorage key bump is required. Bug taxonomy: E retains the existing direct/server import invalidation paths; F is closed by persisted prior state plus explicit recency; D uses existing sign-out cleanup; A/B/C/G/H/I/J/K/L are unchanged because there is no new cache, payload, platform split, or lifecycle behavior.
