...
- **Q4 CAS catalog isolation:** CAS imports can add a server-only provisional `scheme_master` identity and later replace its name through `sync-fund-meta`, but the new `cas_identity_created_at` / `cas_identity_hydrated_at` columns are absent from all client selects. Query keys, fund/transaction payloads, persistence allowlists, stale times, native SQLite rows, and the existing import/sync invalidation paths are unchanged. The provisional identity is not an additional cache: it is a durable referential row whose incomplete status is explicit and retried by the daily/provider-triggered metadata writer. No `__BUSTER__`, SQLite `SCHEMA_VERSION`, Zustand version, or AsyncStorage key bump is required. Bug taxonomy: B/I/L do not apply because no client shape changes; E keeps the existing transaction/fund synchronization; F is handled by explicit provisional provenance plus provider retries; D uses existing user-cache cleanup while the shared catalog remains global; A/C/G/H/J/K are unchanged.
