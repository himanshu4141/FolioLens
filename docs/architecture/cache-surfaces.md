...
- **Q5 direct CAS freshness:** the direct-upload result adds named aggregate outcome fields but no server transaction/fund row, React Query key, or persisted Portfolio payload changes. Added or removed rows mark the existing transaction invalidation fan-out stale on web. Native first runs `syncDeltaForUser`, whose immutable-ID comparison atomically inserts or rebuilds SQLite for either inserts or deletes, then invalidates only input families reported changed. No-op and conflict outcomes run neither sync nor invalidation; hidden routes are marked stale but not refetched. Inbound imports still use initial-session/foreground native sync and the C2 web freshness probe. `__BUSTER__` stays v12 and SQLite `SCHEMA_VERSION` stays 3. The onboarding draft remains at `foliolens-onboarding-draft-v1`: its original `funds` and `transactions` fields retain their meaning, new outcome fields are additive, and the loader repairs old drafts with zero defaults. Bug taxonomy: E is centralized in the existing fan-out; L is explicit web-stale versus native-SQLite-first behavior; B/I are avoided because persisted React Query/SQLite shapes do not change and the backward-compatible draft loader repairs the additive field set; D retains existing sign-out/draft cleanup; A/C/F/G/H/J/K are unchanged.
