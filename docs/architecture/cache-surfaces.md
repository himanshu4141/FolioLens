...
- **Version mechanism:** `__BUSTER__` constant in `queryClient.ts` (currently **v7**). Every persisted entry is keyed by buster; bumping the buster discards every persisted entry on next start. History: v5 added `currentNavDate` to `FundCardData`; v6 added `sebi_category` to `scheme_master`; v7 dropped `morningstar_rating` from the `useSchemeMaster` select (Phase 3 of deprecate-post-openfolio). [cache-shape-stable: period_returns normalisation (PR #206) — `period_returns` typed as `unknown`, always read via `readReturnPct` which handles both decimal and percent shapes; no `select()` columns added/removed, no derived field renamed.] [cache-shape-stable: risk_ratios max_drawdown_5y key addition — risk_ratios jsonb typed as `unknown`, always read via `readOfMaxDrawdown` / `readMfdataStdDev` / `readMfdataBeta` which tolerate absence; reader treats missing key as null; no schema migration needed.] [cache-shape-stable: searchSchemes openfolio_meta_synced_at addition — not in PERSIST_ALLOWLIST, so in-memory cache only; new field is nullable and backwards-compatible on refresh.] [cache-shape-stable: display guards for scheme_master junk — `risk_label`, `declared_benchmark_name`, and `fund_manager` fields typed as `string | null` throughout; new `readRiskLabel` / `readBenchmarkName` / `readFundManager` guards applied at render sites only, never at the query or cache layer; persisted shape unchanged, no `select()` columns added/removed; `fund_manager` added to `ClearLensCompareFundsScreen` local type (already fetched via `SCHEME_MASTER_COLUMNS`) with no new network round-trips.] [cache-shape-stable: payout-plan bypass in `selectCompareMetrics` — no new `select()` columns; `schemeName`/`optionType` params are already fetched in `SCHEME_MASTER_COLUMNS`; `CompareMetrics` interface unchanged; logic-only change skips computed branch for IDCW/payout funds; no persisted data touched.] [cache-shape-stable: OF family identity in Compare Funds — `family_name`, `plan_type`, `option_type` were already in `SCHEME_MASTER_COLUMNS` and `SchemeMasterDbRow` before this change (added in migration `20260509000000`); the new `of_family_id` column is not selected client-side; `ClearLensCompareFundsScreen` now reads `familyName`/`planType`/`optionType` from the already-fetched row rather than computing them client-side; no `select()` columns added/removed; `__BUSTER__` unchanged at v7.] [cache-shape-stable: family-first picker (§6.5) — `v_fund_family_search` view queries (`searchFamilies`, `fetchUserHeldFamilies`) and `fetchFamilyPlans` use query keys `['families', ...]` and `['family-plans', ...]` which are NOT in `PERSIST_ALLOWLIST`; all picker state is therefore in-memory only and evicted on app restart with no stale-shape risk; `ClearLensCompareFundsScreen` `selectedCodes` is still `number[]` and the compare queries (`compare:schemes`, `compare:compositions`, `compare:navhistory`) remain keyed on `number[]`; no `select()` columns were added to or removed from any persisted query; `__BUSTER__` unchanged at v7.] [cache-shape-stable: Compare Funds month-end NAV optimisation — `['fund-nav-history-compare', code]` query now fetches month-end series (~60 rows) instead of 5-year daily series (~1,250 rows), but the cached value type is unchanged (`NavPoint[]`); compare queries are NOT in `PERSIST_ALLOWLIST` (in-memory only), so no stale persisted shape; `__BUSTER__` unchanged at v7.] [cache-shape-stable: as-reported swap guard in `selectCompareMetrics` — logic-only change; no `select()` columns added/removed; `CompareMetrics` interface unchanged; only affects which source branch is chosen when computed Sharpe/Sortino are both null; `__BUSTER__` unchanged at v7.] [cache-shape-stable: cross-category banner equivalence logic — `fundComparisonKey` / `categoriesInSameGroup` added to `src/utils/schemeName.ts` and consumed in `ClearLensCompareFundsScreen`; no `select()` columns added/removed (reads existing `sebiCategory` / `schemeCategory` already in `SchemeMasterRow`); no `CompareMetrics` / cache shape change; `__BUSTER__` unchanged at v7.] [cache-shape-stable: Compare Funds honest labels + as-reported risk shape — `CompareMetrics` gained `trailingPeriodSources` (`{ y1, y3, y5 }` provenance map) in `src/utils/computedFundMetrics.ts`; `CompareMetrics` is NOT persisted in React Query (computed via `useMemo` from `NavPoint[]`, which is in-memory only for compare queries — not in `PERSIST_ALLOWLIST`); `readMfdataStdDev` / `readOfMaxDrawdown` / `readReturnPct` / `readFundManager` are read-side transforms with no effect on the persisted `scheme_master` shape; no `select()` columns added/removed; `__BUSTER__` unchanged at v7.]
...
- **N2T index reuse:** timeline output reads the existing persisted `['index-snapshot', symbol]` entry before fetching a benchmark. The payload remains the canonical `IndexSnapshot` object owned by `useIndexSnapshot`; N2T only filters its `points` to the transaction start date. This changes no key or payload shape and needs no buster bump. A cached `null` snapshot still takes the existing paginated fallback, so a failed CDN response is not treated as authoritative.
- **N3 sync invalidation:** bootstrap and foreground delta sync map `txInserted`, `navInserted`, `idxInserted`, and transaction drift repair to the minimal dependent query-key prefixes in `src/lib/syncInvalidation.ts`. Every affected entry is marked stale with `refetchType: 'none'`; only affected active prefixes owned by the visible route are then refetched. Hidden screen queries include navigation focus in `enabled`, so they remain readable from cache but cannot wake during another route's transition. This changes no query key or payload shape, adds no cache, and requires no `__BUSTER__` bump. Bug taxonomy: E is handled by the centralized dependency map and exhaustive input-family tests; C keeps each query's existing stale time; D uses existing sign-out clearing; B/I/L do not apply because persistence contracts are unchanged.
