...
>
> **sebi_category dirty-data fix (2026-06-24):** 35 active Large & Mid Cap Fund schemes had wrong `sebi_category` values ('mid cap fund' or 'large & mid-cap') because `deriveSchemeCategoryFromName` matched the bare 'midcap' needle before the scheme reached the more-specific 'large & mid cap' patterns. The two missed spellings were "Large & Midcap" (one-word midcap, used by Kotak / Mirae / Nippon / Navi / Union / ITI) and "Large Midcap" (space-separated, used by Edelweiss). Fixed by adding `'large & midcap'` and `'large midcap'` to the first pattern entry in `deriveSchemeCategoryFromName`, adding `SEBI_CATEGORY_ALIASES` normalisation in `resolveSebiCategory` for non-canonical spellings ('large & mid-cap', 'large & mid cap'), and running a corrective UPDATE against dev (35 rows → all now `'large & mid cap fund'`). The Compare screen banner was also made tolerant via `fundComparisonKey` / `categoriesInSameGroup` in `src/utils/schemeName.ts`: non-canonical sebi_category spellings and a null/broad-Equity fallback never trigger the cross-category warning against a specific equity sub-bucket.
...
GitHub Actions cron is reserved for jobs that produce git artifacts (e.g. regression-testing workflows).
