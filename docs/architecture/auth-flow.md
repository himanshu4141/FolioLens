...
So both flows use `https://app.foliolens.in/auth/{confirm,callback}` as the public landing page. The web app at that path bridges into the deep-link scheme (`window.location.replace`) so the native app picks it up via `Linking.useURL()` — but **only for a native-initiated flow**. The decisive signal is the `?scheme=` marker that `getNativeBridgeUrl` appends (GoTrue preserves it through the OAuth/magic-link redirect); the bridge additionally requires a mobile user agent and that the current host is the configured `EXPO_PUBLIC_APP_BASE_URL` host. This decision lives in the pure `shouldBridgeToNativeApp` helper (`src/utils/appScheme.ts`), shared by both `auth/callback` and `auth/confirm`.

A flow **without** `?scheme=` — every web-initiated sign-in, whether on desktop or mobile — must serve regular auth UI instead: the magic-link "check your inbox" screen, or `detectSessionInUrl()` + redirect to home. This matters because the web app and the native-bridge host are the **same** host in both envs (`app.foliolens.in` in prod, `foliolens-dev.vercel.app` in dev), so a mobile-web sign-in lands on the bridge host too. Bridging it on user-agent alone (the pre-`shouldBridgeToNativeApp` bug) `replace`d a mobile-web visitor into `foliolens://`, which opens nothing on a device without the app and aborts the in-flight web session exchange — leaving them stuck on the callback screen, unauthenticated.
