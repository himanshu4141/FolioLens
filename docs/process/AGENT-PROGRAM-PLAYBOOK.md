...
- **The source of truth is SHA-pinned comments, not labels.** The gate passes only
  ...
  the **current** head. A signature footer that the reviewer's posting
  integration appends automatically (e.g. Claude Code's
  `_Generated by [Claude Code](…)_`) is stripped by the gate before matching and
  is therefore tolerated; any **other** trailing text is not. Whole-body matching
  is deliberate: substring matching would accept negations
  ("NOT CONVERGED at <sha>", "Withdrawing CONVERGED at <sha>") or quoted mentions
  of the format in discussion. A push instantly invalidates convergence because
  the pinned SHA no longer matches — no label state can go stale into a green
  gate, and the first program's "merged before the final convergence marker" slip
  cannot recur.
- **Withdrawing a convergence at the same head:** edit or delete the marker
  comment — or, for integrations that cannot edit or delete their own comments,
  post a standalone comment whose entire body is exactly
  `[<Role> review <ID>] WITHDRAW CONVERGED at <full 40-char head SHA>`. A
  CONVERGED marker counts only while it is newer than every WITHDRAW marker for
  the same role and head; re-converging after a withdrawal requires a **new**
  marker comment (timestamps are creation times, so editing an old comment
  cannot resurrect a withdrawn convergence).
- **The merged gate is authoritative over this document.** If the gate's behavior
  and this section disagree, the gate wins, and any PR that changes
  `program-convergence-gate.yml` must amend this section in the same PR. This
  rule exists because drift caused a real deadlock: the gate learned footer
  tolerance in PR #264 while this document still said "no footers", and the
  executor enforced the stale text against a gate-valid marker on N7.
...
5. Decide the merge authority. For a fully automated program the executor merges
   ...
   merge for the first milestones until the loop has a track record. Also name
   the **closeout owner** — the session that runs §5.5 when the queue completes
   (default: the executor; the first program assigned it to the Codex reviewer
   because that session authored the report). Record both decisions in the
   control PR description.
...
1. **Pause the queue.** No new milestone starts until the fix merges. Mark any
   in-flight milestone row `Paused`. The human owner may explicitly re-sequence —
   e.g. let an already-in-review milestone land first, as the first program did
   when C2 was confirmed mid-N8 — but the decision must be recorded as a control-PR
   comment, and program closeout stays blocked until the interrupt merges.
...
Closeout is run by the **closeout owner** named at setup (§5.2), using the §7.7
prompt: verify every merge SHA on `origin/main`, evaluate the exit criterion,
update the report with final outcomes and accepted unmet conditions, reconcile the
tracking table and ledger, post one final closeout comment, then mark the control
PR ready and merge it. If the exit criterion needs a post-merge observation window
(field telemetry over days or weeks), schedule that check before closing the
program; a regression inside the window is a new correctness interrupt.
...
- Control PR comments: **two per milestone** (START, MERGED) + hotfix investigation
  ...
  recorded owner decisions (merge authority, interrupt re-sequencing) and the final
  closeout comment. Everything else belongs on the implementation PR. No
  "acknowledged", no status narration.
...
### 7.4 Codex reviewer session (given once, runs the whole program)
    ...
    3. Review that frozen head under the stance and non-negotiables below. File
       ...
       an auto-appended integration signature footer is tolerated (the gate
       strips it), any other trailing text is not — plus the codex-converged
       label. The gate accepts only that exact whole-comment format at the
       current head; the label add is what re-triggers it. To withdraw a
       convergence at the same head, edit or delete the marker comment, or if
       your integration cannot edit/delete, post a standalone comment whose
       entire body is exactly "[Codex review <ID>] WITHDRAW CONVERGED at
       <full 40-char head SHA>"; re-converge later with a NEW marker comment.
...
### 7.5 Claude reviewer session (given once, runs the whole program)
    ...
    3. Review that frozen head under the stance and non-negotiables below. File
       ...
       an auto-appended integration signature footer is tolerated (the gate
       strips it), any other trailing text is not — plus the claude-converged
       label. The gate accepts only that exact whole-comment format at the
       current head; the label add is what re-triggers it. To withdraw a
       convergence at the same head, edit or delete the marker comment, or if
       your integration cannot edit/delete, post a standalone comment whose
       entire body is exactly "[Claude review <ID>] WITHDRAW CONVERGED at
       <full 40-char head SHA>"; re-converge later with a NEW marker comment.
    4. The executor is barred from pushing while a round is open, so the head
       you review is stable. If a mid-round push happens anyway, the round is
       void: stop, discard round conclusions, and wait for the re-review label.
...
### 7.7 Program closeout (for the closeout owner named at setup)

    Every milestone and interrupt in the {{PROGRAM_NAME}} tracking table is
    Merged. You are the closeout owner. Run the following, in order:

    1. Verify every tracking-table row's merge SHA is present on origin/main.
    2. Evaluate the report's "Program exit criterion" against real evidence.
       Record the outcome in the report: met (with the evidence), or explicitly
       accepted as unmet by the human owner (with the acceptance and reason).
       Do not substitute per-milestone acceptance evidence for the exit
       criterion.
    3. Update the research report with final per-milestone outcomes, deviations,
       and accepted unmet conditions; push to the research branch.
    4. Reconcile the control PR description: tracking table complete, ledger
       consistent with the report, no dangling Paused/In-progress rows.
    5. Post ONE final closeout comment on PR #{{CONTROL_PR}} summarizing the
       verified state, then mark the control PR ready and merge it as the
       historical record.
    6. If the exit criterion requires a post-merge observation window (e.g.
       production telemetry over N days), schedule or hand off that check with
       an explicit date before closing. A regression inside the window is a new
       correctness interrupt, not a debate.
...
9. **Review in frozen-head rounds.** If the executor can push while one reviewer
   ...
   next round.
10. **The gate and this document must move together.** PR #264 taught the gate to
   tolerate an auto-appended reviewer footer but left this document's "no
   footers" text in place; on N7 the executor enforced the stale prose against a
   gate-valid marker and stalled a merge that was already clean. The merged gate
   is authoritative, and any gate change amends §4 in the same PR.
11. **Don't assume reviewers can edit or delete their own comments.** The Claude
   posting integration can do neither, which broke the original
   "edit-to-withdraw" design and forced footer tolerance. Every protocol step a
   reviewer must perform needs to be achievable by posting a new comment.
