...
The dry-run and apply SQL qualify the hosted cryptographic digest dependency explicitly. Temporary CLI sessions must not rely on a role-specific `search_path`, because same-session role assumption does not apply per-role startup settings. Qualification changes only dependency resolution; the SHA-256 inputs, ordering, and manifest contract remain identical.
...
Install Supabase CLI 2.114.0 or later, log in with `supabase login`, and keep Docker running. The owner-facing C2/C3 wrapper reads the selected CLI profile from the macOS Keychain, obtains the exact shared-dev pooler plus a five-minute `cli_login_postgres` role from the Supabase Management API, and runs psql in an auto-removed `postgres:17-alpine` container. In exact `cli-temporary` mode, the reviewed adapter enables command-line `ON_ERROR_STOP` and assumes exactly `postgres` before the existing command or SQL file in the same psql session. It refuses caller-supplied database host, user, port, name, password, psql adapter, or PostgreSQL startup-option overrides before credential access. No database password file is needed or permitted in this path.
...
`SUPABASE_PROFILE` is optional and defaults to `supabase`. The low-level `run-exact-target-repair.sh` remains the reviewed implementation and direct-password compatibility entry point, but the field repair must use `run-exact-target-repair-with-cli.cjs`. The wrapper passes the temporary database password, exact import ID, approved manifest, approval phrase, and service-role credential only through child-process environments or existing mode-0600 temporary configuration, never command arguments. The fixed role command is static and non-secret. Its harmless `probe` mode runs one silent, staged, zero-row permission-sensitive script and prints only `{"cli_transport":"ready"}`.
...
The CLI wrapper validates the hard-coded shared-dev project, official primary pooler suffix, project-scoped pooler user, exact temporary-role name, server-reported lifetime, and session-mode port before starting the low-level runner. It requires exact effective role `postgres`, zero-row permission analysis for all three repair tables, execute authority for the exact activation-resolver signature, and a successful fixed empty-string call to the qualified manifest digest dependency in the same psql session as the fixed role assumption. The digest probe reads no repair rows and proves both schema usage and function execution before the runner starts. Missing role membership, incomplete authority, inaccessible digest dependency, or any caller override stops before the low-level runner; command-line `ON_ERROR_STOP` prevents a later command or SQL file from running after failed role assumption. The adapter injects these arguments only when `Q5_REPAIR_AUTH_MODE` is exact `cli-temporary`; password mode retains its prior psql argument behavior. There is no automatic weaker fallback or ad hoc role bypass. The wrapper anchors the expiry deadline before the bounded login-role request and stops the complete detached runner process group before that conservative deadline. Operator `SIGINT` or `SIGTERM` is forwarded to that group, and the wrapper remains alive until group shutdown is confirmed. A timed-out readiness probe is retried only while the temporary-role deadline still permits it; version, keychain, permission, and other process-start failures remain fail-closed. Keep the encrypted backup and key at different local paths outside the repository. Do not place either in a cloud-synchronized directory.
