...
| `sync-nav` | pg_cron (bimodal: hourly 6 PM → 6 AM IST + every 2h during the day, 7 days) | Calls OpenFolio `/health` first and skips schemes already current through `db_nav_latest`; only missing/stale held schemes use the OpenFolio-first (`since=` incremental), mfapi fallback ladder | Active |
