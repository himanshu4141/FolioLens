...
| `sync-nav` | pg_cron (bimodal: hourly 6 PM → 6 AM IST + every 2h during the day, 7 days) | Calls OpenFolio `/health` first and skips schemes already current through `db_nav_latest`; missing/stale held schemes use OpenFolio `/v1/nav/delta` in one selected-scheme request, with per-scheme OpenFolio and mfapi fallback only for rollout gaps or missing/truncated schemes | Active |
