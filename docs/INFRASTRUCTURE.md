...
| `cas-webhook-resend` | Vercel inbound router | Receives FolioLens-signed CAS payloads routed by `/api/resend-inbound-router`, looks up user via `cas_inbox_token`, fetches email content / attachments through Resend, parses and preflights every PDF, then combines the validated folios into one atomic import plan so the whole message commits or rolls back together | Active |
...
| `sync-fund-meta` | pg_cron (daily) + CAS provisional-identity trigger | Refreshes held-fund scheme metadata from OF `/v1/metadata` (AUM, expense ratio, risk, family_name, plan_type, option_type, of_family_id), includes CAS-created identities until canonical hydration succeeds, obtains canonical AMFI name/ISIN from mfapi, then fills unresolved B1 fields from mfdata.in. OF values take precedence; mfdata only writes fields still unresolved after the OF pass. | Active |
