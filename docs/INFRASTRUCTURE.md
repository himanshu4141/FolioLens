...
| `universe-backfill` | GitHub Actions scheduled (every 15 min) + manual `workflow_dispatch` | Bulk-syncs OpenFolio composition + metadata for **full active AMFI universe** (~37,595 schemes), not just held funds. Chunked invocations (2 pages per run), cursor state + done markers in `app_config`. Supports `force=true` for OP-1 remediation re-runs. See docs/universe-backfill-implementation.md. | OP-1 (PR #202) |
