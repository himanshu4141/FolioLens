...
5. **OpenFolio NAV age** — `db_nav_latest` from the same `/health` response must be within 3 calendar days on a weekday (skipped on weekends — AMFI doesn't publish NAVs then). Independent of check 4: OpenFolio can report `status='ok'` with a healthy `db_schemes` count while its own NAV ingest is frozen, which is exactly what happened in the 2026-09 incident. Failure: OpenFolio is healthy-but-stale.
...
request and any redirect back through the proxy are accepted too. As of D4, the
**PROD** client lists its matching proxy callback the same way, added by the
human owner together with the rest of D4's batched production-console changes
(recorded in the ExecPlan's Decision Log):
...
| FolioLens | `https://ohcaaioabjvzewfysqgh.supabase.co/auth/v1/callback`, `https://api.foliolens.in/auth/v1/callback` (added D4) |
...
- **PostHog** — single pane for product events and operational health, fed from every surface that runs FolioLens code:
  ...
  - **Client UX performance**: explicit low-cardinality timing and cache-health events `navigation_performance`, `ux_screen_ready`, `ux_interaction_latency`, `ux_slow_event`, `ux_js_stall`, `ux_cache_health`, `perf_mark`, `persister_restore_completed`, `persister_restore_failed`, `db_sync_complete`, and `tx_cache_reconciled`. These intentionally avoid autocapture/session replay and never include user/fund/transaction IDs, route pathnames, financial values, or exact sync counts. Database sync reports only bucketed insert/error counts and mode; transaction reconciliation reports only bucketed local/server/drift counts, drift direction, mode, and whether repair completed. Dashboard by `surface`, `readiness`, `source_event`, `platform`, `app_version`, and `eas_update_id`; alert on p95 `ux_screen_ready.elapsed_ms`, sustained `ux_slow_event` spikes grouped by `source_event`, and `ux_cache_health.blob_size_bucket = 5MB+`.
