...
- 2026-08-23: Field-proof preparation showed that the temporary `cli_login_postgres` login authenticates with `NOINHERIT`; C2's `select 1` probe therefore proved connectivity but not the protected-table authority required by Q5. A fixed libpq startup option passed disposable PostgreSQL validation but failed closed through the exact-dev session pooler. C3 owns the separately reviewed correction: reject inherited startup options without forwarding them; in exact CLI-temporary mode inject command-line `ON_ERROR_STOP` followed by one fixed same-session `SET ROLE postgres`; and require exact effective role plus zero-row permission analysis for all three repair tables and resolver execute authority before starting the low-level runner. The five Q5 SQL files and direct-password compatibility path remain unchanged.
...
- C3 supersedes the underpowered C2 readiness evidence. See `docs/plans/cas-import-correctness-c3.md`; Q5 field proof remains paused until that correction merges through exact-SHA dual review.
...
- [x] Freeze one exact re-review head.
- [x] Obtain exact-SHA Codex and Claude convergence and merge only on the green gate.
