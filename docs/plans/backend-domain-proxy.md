...
**Production release, owner-performed (2026-07-17):** the human owner tagged and pushed `v0.0.9` at `main`'s then-current head (`9045365`, D3's merge commit — PR #284 itself is docs-only, so its merge state doesn't affect release content). `production-release.yml` run [`29579972299`](https://github.com/himanshu4141/FolioLens/actions/runs/29579972299) completed successfully (typecheck/lint/test, EAS OTA to `foliolens-production`, Vercel prod deploy). The owner then tested production directly on both web and a production-channel native build and confirmed sign-in and normal operation on both ("everything is good. Test passed!"). Independently verified by the executor: fetched the live `app.foliolens.in` bundle post-release — zero occurrences of the raw PROD Supabase host (`ohcaaioabjvzewfysqgh.supabase.co`), four occurrences of `api.foliolens.in`. This satisfies D4's acceptance criteria in full (production release workflow succeeded; production web and a production-channel build authenticate and load a portfolio with no regression).

This happened concurrently with PR #284's review round 1: both reviewers correctly flagged (at head `13db886`) that production hadn't cut over yet at that point, and Claude relayed an owner-approved plan to formally rescope D4 to prep/staging-only with the actual cutover deferred to a later milestone. That rescope is superseded by the owner completing the real cutover directly (out-of-band from this PR, since the release doesn't depend on it merging) before the rescoped plan could land — see Decision Log. D4's originally written acceptance criteria stand as genuinely met, with the evidence above.
...
- Risk: the three accepted residuals (the OAuth provider-callback leg, the JWT `iss`, the magic-link email verification link) are mistaken for defects and someone attempts to "finish the job" by buying the Custom Domain add-on or otherwise routing around them. Mitigation: documented explicitly in this plan and `docs/INFRASTRUCTURE.md` as accepted, not open. The research report documents only the original two residuals, and `docs/EXIT-RUNBOOK.md` has no residual-specific text yet; bringing both up to the same three-residual shape is D5-scoped doc closeout, not yet done.
...
- 2026-07-17 (D4 review round 1, Codex, PR #284): Codex correctly flagged (P1) that D4 had not actually cut production traffic over yet at frozen head `13db886` — the latest `production-release.yml` run was still `v0.0.8` (2026-07-14), and the live prod bundle still showed the raw PROD Supabase host. Requested either completing the release + smoke evidence at that head, or an explicit owner/control-plane scope amendment moving the actual cutover acceptance to a later milestone.
- 2026-07-17 (D4 review round 1, Claude, PR #284): Concurred with Codex's P1 and relayed an owner-approved scope amendment: formally rescope D4 to prep/staging-only (Worker deployed + every prod console/env/secret/OAuth/allowlist change staged), moving the actual production release + smoke test to a follow-on milestone. Withheld `CONVERGED` pending the rescoped plan. Also flagged (secondary) that the Risks section's residual-documentation claim overstated what `docs/EXIT-RUNBOOK.md` and the research report currently say — fixed above.
- 2026-07-17 (D4, superseding the round-1 rescope): before the rescoped plan could land, the human owner tagged and pushed `v0.0.9` directly and tested production on both web and native — both passed. This is a real, independently-verified production cutover (run `29579972299` succeeded; live bundle confirmed proxy-only), so D4's originally written acceptance criteria are genuinely met rather than rescoped down. The Risks-section wording fix (above) still stands on its own merits, independent of this.
...
- [x] D4 — shipped via `production-release.yml` (tag `v0.0.9`, run `29579972299`, success); production web + native smoke-tested by the human owner, independently spot-checked by the executor (live prod bundle: zero raw-host occurrences, four proxy-host occurrences).
