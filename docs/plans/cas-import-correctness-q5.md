...
The live repair is deliberately split from the merged application code. A generic committed runbook defines the invariants and accepts the exact import identifier only through a non-echoed runtime channel. The local wrapper connects only to the authorized shared-dev database, passes database credentials and private selectors through the child environment rather than process arguments, proves every target row belongs to the audit owner, computes target and unrelated digests, and streams the target rows into locally encrypted mode-0600 backup storage. It emits only aggregate counts and digests. Mutation is disabled unless the fresh approval phrase is supplied after the human reviews the dry-run manifest. The mutation executes one serializable database transaction that locks the transaction table, loads the decrypted backup into a temporary table, proves its count, owner, import scope, and complete-row digest match the approved target, rechecks current target and unrelated digests, deletes only `cas_import_id = target`, validates the deleted count and unrelated digest, and commits. Any mismatch rolls back. Apply and rollback delete their mode-0600 plaintext on every exit. A separate rollback mode restores only the backed-up primary keys after proving none already exist and every row still belongs to the audit owner.
