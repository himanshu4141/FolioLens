...
- Advisory-lock review correction evidence: Claude converged on the C1/Q5 integration head while Codex reproduced one v3 snapshot race. The wrapper now takes the exact sorted v2 advisory locks before its before-state reads. A committed dblink regression held the key and changed activation in one session, started v3 in another, proved v3 waited on the advisory lock before reading, then proved the duplicate-only import returned `holding_changed_count = 1` after the writer committed. The freshly replayed migration chain and all three pgTAP files pass 49 assertions; full Jest passes 113 suites / 2,262 tests; focused Jest, typecheck, zero-warning lint, schema lint at error level, and diff checks pass. The isolated stack was destroyed, and neither shared data nor private statements were touched.
