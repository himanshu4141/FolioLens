...
- 2026-08-24: Codex and Claude independently found the same sole round-one P1 at `fdeee920a60b4f3fdee317486b0eed8d11272d58`: the fatal CLI adapter emitted the successful role-command tag before backup CSV and hydration JSON. The batched correction adds quiet mode before command-line `ON_ERROR_STOP`, retains copy output, and adds stream-level regressions for both consumers. Validation passes 2 focused suites / 43 tests, 114 full Jest suites / 2,293 tests, typecheck, zero-warning lint, shell and Node syntax, diff checks, and unchanged Q5 SQL.
...
- [x] Open the draft correctness-hotfix PR and freeze one exact review head.
