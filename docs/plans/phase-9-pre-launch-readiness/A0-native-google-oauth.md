...
- Corrected-head Android evidence distinguishes what can be forced natively from what is proven deterministically. Android delivered a crafted fragment to the callback route without exposing its hash to the Router input, so the route timed out actionably and no session was created or replaced; the coordinator's explicit `unsupported_fragment`, stale-`/auth/confirm`, and reconciliation exact-once branches remain covered by deterministic regressions rather than being overstated as native branch coverage.
...
- The initial `c729b14` Android record above is superseded for merge acceptance because independent review required runtime corrections. Corrected runtime head `c61d633adaaff33dad6cc7b1bb7200e003708273` includes all four fixes: lifecycle publication on reconciliation, one-shot suppression of a delayed identical provider transition, installed-scheme plus exact-`/auth/callback` fallback validation, and rejection of every OAuth token fragment before provider mutation.
- Corrected-head validation: focused 3 suites / 36 tests, typecheck, zero-warning lint, full Jest 88 suites / 1,915 tests, diff check, cache-shape gate, and the convergence gate passed. PR Preview run `28756412082` passed both jobs and published Android update `019f3452-0a35-75a0-b7ce-ea670aefb091` plus iOS update `019f3452-0a35-74d8-baf7-ad510fa4f156` for exact commit `c61d633a`.
- Corrected Pixel 8a setup: About displayed `019f3452-0a3...`; app data was then cleared, the exact update redownloaded (`DownloadComplete` named the full Android ID), the app restarted, and About displayed the same prefix after authentication.
- Corrected clean first attempt: one Google tap and one account selection reached Portfolio without an app restart or second sign-in tap. Flow `oauth-mr8cprns-1` emitted `oauth_started` at 0 ms, `callback_received` at 10,021 ms, `session_started` at 10,024 ms, `browser_returned` at 10,026 ms, `session_confirmed` at 11,701 ms, and `navigation_completed` at 11,745 ms. Callback source was `router`, transport was `code`, and browser result was `success`. The associated cold SQLite bootstrap completed in 120,507 ms with 159 transaction, 47,929 NAV, and 15,611 index rows inserted and `error_count: 0`.
- Corrected session persistence: after the cold bootstrap completed, force-stop/relaunch returned directly to Portfolio with no Google action. Its cache-warm bootstrap completed in 6,605 ms with zero inserted rows and `error_count: 0`.
- Corrected cancellation and background/foreground retry: custom-tab close returned in 5,355 ms and emitted terminal `browser_dismiss` at 5,359 ms; loading cleared and the enabled retry screen stated that Google sign-in was cancelled. On retry, Home -> Recents -> FolioLens PR preserved the chooser. Flow `oauth-mr8cumrd-2` then emitted callback at 19,436 ms, session start at 19,445 ms, browser return at 19,448 ms, session confirmation at 21,016 ms, and navigation at 21,044 ms. Its post-sign-out full rebuild completed in 137,463 ms with 47,929 NAV and 15,611 index rows inserted and `error_count: 0`.
- Corrected hostile-callback evidence: while authenticated, a crafted `foliolens-pr://auth/callback` token fragment left the same Portfolio session visible. After sign-out, the same deep link could not establish a session and ended on the callback route's actionable `FolioLens did not receive the Google callback` retry state. Expo Router did not expose the hash to the native callback input, so this native case proves no session mutation but not execution of the coordinator's `unsupported_fragment` branch; focused regressions prove cold and in-flight fragments are rejected before exchange/session mutation. The stale-confirm fallback and forced reconcile -> delayed identical provider-event ordering are likewise deterministic-test evidence; the physical run exercised the normal provider-event path once per successful sign-in.
- Corrected privacy and failure scan: 15 captured `[auth/oauth]` records contained no callback URL, authorization code, access/refresh token, email, or provider-identity fields. All three measured bootstraps reported `error_count: 0`; the app processes emitted zero auth-lifecycle, SQLite, storage, React Native fatal, or unhandled-promise errors. The temporary screen timeout was restored to 120,000 ms.
...
- [x] Open the ready implementation PR.
- [ ] Obtain Codex/Claude convergence on the corrected runtime and Android evidence.
