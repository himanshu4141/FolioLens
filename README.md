...
- **Agent QA workflows** — repo-local Codex plugin `plugins/foliolens-qa` packages namespaced PR QA, production smoke, and full regression skills; the Claude plugin lives under `qa/claude/foliolens-qa`. Both share the canonical FolioLens route, cache, theme, and report reference in `docs/qa/foliolens-app-reference.md`.
