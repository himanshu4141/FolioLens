...
- **Authoritative historical NAV cache** — native SQLite records per-scheme lower-bound coverage separately from row presence, so Portfolio timelines, Fund Detail, and Compare never treat a recent NAV slice as complete history. Unknown intervals repair once from the upstream source; subsequent reads stay local, including authoritative empty pre-allotment periods for new fund offers.
...
- **Persistent local cache** — React Query persists to `localStorage` (web) / AsyncStorage (native) so a page reload paints Portfolio + chart from disk before the network resolves. Per-user raw inputs (`['user-funds', userId]`, `['user-transactions', userId]`) and per-scheme metadata (`['scheme-master', code]`, `['fund-nav-history', code]`) are shared across screens via `qc.fetchQuery` — once Portfolio has loaded the user's funds and transactions, Fund Detail and Compare resolve those in zero network round-trips. Investment timelines keep benchmark-independent transaction/NAV lookups and bounded portfolio valuations in a user/fund-set/window memory entry, so changing only the benchmark reuses the portfolio side and performs index work only. Native SQLite writes share one connection-level queue, so bootstrap, foreground refresh, and read-through repair cannot open overlapping transactions; failed timeline NAV repair retries from retained rows before the query reports success. Native bootstrap, foreground refresh, and sign-out cleanup run even when analytics is unavailable, so telemetry configuration can never disable cache correctness or privacy cleanup. Field-measured Fund Detail second-open: **397ms vs 1282ms on first open** (~3.2× speedup). A `useIsRestoring` gate on Portfolio + Fund Detail collapses the rehydrate window so the screen never flashes "Import CAS" before the cached data arrives. Bump `__BUSTER__` in `src/lib/queryClient.ts` to invalidate after a query-shape migration.
- **Navigation performance controls and telemetry** — release builds record privacy-safe press-to-route-commit and press-to-post-interaction timings for Portfolio → Settings, Settings → About, Funds → Fund Detail, and bottom-tab switches. Native tab, Settings, and financial-tool screens freeze on blur, expensive screen-owned queries are disabled while hidden, and background synchronization marks only transaction/NAV/index-dependent query families stale before refetching the affected visible route. Alternate benchmarks are never computed speculatively: pills prefetch only the option receiving press/hover/focus intent, and Fund Detail reads portfolio weight from the existing cache without starting a second full portfolio aggregation. Same-label timing work uses independent span IDs, and navigation events contain only fixed route names, cache/sync state, active-query count, and aggregate row counts. Baseline collection is documented in [`docs/performance/navigation-baseline.md`](./docs/performance/navigation-baseline.md).
