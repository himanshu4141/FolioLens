...
- **Persistent local cache** — React Query persists to `localStorage` (web) / AsyncStorage (native) so a page reload paints Portfolio + chart from disk before the network resolves. NAV / index history reads through a shared cache that delta-fetches on revalidation: a returning user only pulls today's NAV tick (~10 rows) instead of the full 12,500-row history. Bump `__BUSTER__` in `src/lib/queryClient.ts` to invalidate after a query-shape migration.
