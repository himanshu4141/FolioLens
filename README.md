...
- **Persistent local cache** — React Query persists to `localStorage` (web) / AsyncStorage (native) so a page reload paints Portfolio + chart from disk before the network resolves. Per-user raw inputs (`['user-funds', userId]`, `['user-transactions', userId]`) and per-scheme metadata (`['scheme-master', code]`, `['fund-nav-history', code]`) are shared across screens via `qc.fetchQuery` — once Portfolio has loaded the user's funds and transactions, Fund Detail and Compare resolve those in zero network round-trips. Investment timelines keep benchmark-independent transaction/NAV lookups and bounded portfolio valuations in a user/fund-set/window memory entry, so changing only the benchmark reuses the portfolio side and performs index work only. Native SQLite writes share one connection-level queue, so bootstrap, foreground refresh, and read-through repair cannot open overlapping transactions; failed timeline NAV repair retries from retained rows before the query reports success. Field-measured Fund Detail second-open: **397ms vs 1282ms on first open** (~3.2× speedup). A `useIsRestoring` gate on Portfolio + Fund Detail collapses the rehydrate window so the screen never flashes "Import CAS" before the cached data arrives. Bump `__BUSTER__` in `src/lib/queryClient.ts` to invalidate after a query-shape migration.
