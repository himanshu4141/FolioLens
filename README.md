...
- **Navigation performance controls and telemetry** — release builds record privacy-safe press-to-route-commit and press-to-post-interaction timings for Portfolio → Settings, Settings → About, Funds → Fund Detail, and bottom-tab switches. Alternate benchmark work is focus-gated and cancelled when its screen blurs, benchmark pills prefetch only the option receiving press/hover/focus intent, and Fund Detail reads portfolio weight from the existing cache without starting a second full portfolio aggregation. Same-label timing work uses independent span IDs, and navigation events contain only fixed route names, cache/sync state, active-query count, and aggregate row counts. Baseline collection is documented in [`docs/performance/navigation-baseline.md`](./docs/performance/navigation-baseline.md).
